...
- [x] 操作者点击入口后正常候选转盘加载；该条验证目标组候选 UI 成功，不推断候选数量
  边界或 Network 面板中的原始响应。
...
- [x] 操作者完成首次抽签；随后再次从「转一下」入口进入时恢复上次选中结果，没有作为
  新一轮重新随机。
- [ ] 点击抽签后业务结果立即锁定，约 3 秒动画停止在同一候选；无第二次随机。
- [ ] 首次抽签后模式不可更改。
- [x] 首次抽签后只允许一次「再转一次」；第二次结果正常出现，此后无法继续重转且
  界面明确呈现次数耗尽状态；耗尽结果页不再显示模式选项。
- [x] 排除仅从本轮候选移除并重新计算概率，且没有恢复抽签机会；代码路径只保存本地
  wheel session，生产只读复核仍为 3 家 active 餐厅，没有永久删除餐厅或写入长期偏好。
