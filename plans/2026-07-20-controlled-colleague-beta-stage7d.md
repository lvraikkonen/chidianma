...
完成状态：Popup 保留既有 `PopupViewState` 与推荐、缓存、QuickAdd、错误流程，只在
fresh `ready / empty` 且 Server capability 开启时显示「给我推荐 / 转一下」入口；点击
转盘前重新校验当前 group/membership storage snapshot，返回推荐不发起 refresh。新增纯
`wheelPopupModel` 和 `wheelSpinLifecycle`，扇区严格使用累计概率，盘内只放编号，结构化
列表显示真实名称、签数和概率；UI 不生成第二个随机结果。正常模式以固定 3 秒 timer
完成已预选扇区动画，reduced-motion 直接完成同一结果，均不依赖 `transitionend`。

Popup 已接通默认 weighted 原生 radio、首次抽签后锁定、一次重转、会话排除、接受与
恢复结果；Server 候选刷新和会话写入期间统一锁定模式与结果操作并设置 `aria-busy`，
避免 UI 模式与实际抽签模式竞态。结果仅通过启动时已存在的 live region 播报并聚焦
`tabindex=-1` 标题；接受成功后把已验证的 participation response 同步回既有 Popup
state，返回推荐不需要刷新也能看到最新决定。结果只使用 Server 候选字段。
`demo-design/popup.html` 的双入口、
暖色转盘、指针、主文案与结果卡层级被吸收；静态候选、硬编码中奖、假模式按钮、
静态距离/近期天数、通用餐厅图片、大幅吉祥物和长期学习承诺均未带入。Extension
版本提升为 `0.3.0`，ID、权限和 host 不变。
