...
Status: `In Progress; Stage 7A–7C Complete, Stage 7D.0 Baseline Frozen`
...
`v0.1.0-internal` 标签保留。Stage 7D 基线为提交
`072ce70abda268f2cdf4fea1a349c16a976e70b5`，由已推送的 annotated tag
`v0.2.0-internal` 冻结。当前生产运行时仍为 Stage 7C Railway 部署
`03d744f6-a5bd-486c-ba65-3541dbfe9096`，来源提交为
`e9912c9cc72e237b0baa1aa922b3f49c5473f66a`；`072ce70` 只增加文档，Railway 将该次
部署标记为 skipped。运行时使用部署 ID、来源提交和镜像 digest 追踪，不把基线 tag
误写成线上运行制品。
...
| Stage 7C | 品牌、图标、体验一致性与受控 unpacked 分发 | 是 | Done：自动化、Railway、真实 Chrome 与打包退出门均通过 |
| Stage 7D | 受控功能切片、小范围同事内测、反馈和账号体系决策 | 否，属于内测过程 | In Progress：7D.0 完成，7D.1 待实施 |

执行必须保持 7A → 7B → 7C → 7D 的顺序。7A、7B、7C 的退出门未通过前，
不把版本交给普通同事作为正式内测入口。必要的开发者或审查者验证不算进入
Stage 7D。

Stage 7C 完成只批准以 `0.2.0` 候选作为 Stage 7D 规划输入。Stage 7D 的当前范围和
执行门由
[`2026-07-20-controlled-colleague-beta-stage7d-design.md`](2026-07-20-controlled-colleague-beta-stage7d-design.md)
以及对应详细计划定义；实际同事分发仍需明确 cohort 批准。
...
- 统一 Admin、Popup、设置页和独立详情页；修复详情页原生按钮、品牌标记、重复设计
  token、内部 `Stage 5C` 文案、固定生产 API onboarding 和重复 `<h1>`。
- 补齐 Admin Modal 键盘 focus trap，并让 Admin/Extension QuickAdd 的 lost-response
  retry 不会静默创建重复推荐。
- 按 `decisions/0002-extension-distribution.md` 的已接受决策使用版本化 unpacked 包；
  Chrome Web Store unlisted、listing 和宣传素材延后到首轮内测反馈之后。
- 产出版本化扩展构建物、校验值、安装/升级/卸载说明、权限说明和支持核对信息。
  构建物不得包含真实邀请码、Token、Secret 或开发地址。
- 进行真实 Chrome unpacked 安装、覆盖升级、稳定 ID、通知、详情页、390px Admin 和
  ZIP 回滚烟测；Chrome Web Store 延后到首轮同事内测之后再决策。
...
Stage 7D 是内测过程，不阻塞内测启动。当前 Stage 7D 规格在不改变核心产品边界的
前提下，先增加幸运餐厅大转盘，再以独立 spike 验证附近餐厅候选搜索与辅助录入；
完整行为、provider policy、feature flag 和分支门以 2026-07-20 的当前 Stage 7D 规格
和计划为准。先限定小规模真实同事和明确时间窗，再扩大：
